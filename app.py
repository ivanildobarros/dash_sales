        ...
        dbc.Col([
            dbc.Card([
                ...
                dcc.RadioItems(
                    ...
                    "height": "90vh", "margin": "20px", "padding": "20px",
                })
        ], sm=2),
        dbc.Col([
                dbc.Row([
...
def render_graphs(cities, main_variable):
    ...
    df_income_time = df_filtered.groupby("Date")[
    ...
    fig_payment = px.bar(df_payment, y="Payment",
                         x=main_variable, orientation="h")
    fig_gender = px.bar(df_gender, y=main_variable, x="Gender", color="City", barmode="group")
    fig_product_income = px.bar(df_product_income, x=main_variable,
                                y="Product line", color="City", orientation="h", barmode="group")
    fig_income_date = px.bar(df_income_time, y=main_variable,
                                x="Date")
    ...
    fig_product_income.update_layout(margin=dict(
        l=0, r=0, t=20, b=20), height=500)
